...
You will often need to check the data type of objects and convert them to other types. To handle this, R has a series of functions that follow a pattern:

- Check the type with `is.______()`
- Convert the type with `as.______()`

In each of these patterns, replace "`______`" with a data type, such as `character`, `numeric`, or `logical`.
