...
In this case, R assumes that `3` is really `3.0`. To make sure R knows you really do mean to create an integer, you have to either add an `L` to the end of the number or use the `as.integer()` function to _coerce_ the number into an integer type:
...
For both integers and floats, the `class()` function will tell you that both are `numeric` data types:
...
In general, you can check if an object is a specific data type using `is.something()`, replacing "`something`" with a data type:
...
One common problem with this approach that you'll frequently run into is the use of `is.integer()`. If you use `is.integer()` to check if a number that **looks** like an integer, you might get a surprising result:

```{r}
is.integer(7)
```

This is because the default numeric type is a _double_. The safer way to check if a number is an integer in value is to compare it against itself as an integer:
...
Keep in mind that some things don't make sense to be converted. For example, if you try to coerce a character word into a number, R will return `NA`:

```{r}
as.numeric('foo')
```

If you want to force a non-integer number into an integer value, but you don't want to change the actual _type_ to an integer type, use `round()`, `ceiling()`, or `floor()`. This will turn the number into an integer in _value_, but will still technically be a `double` type. Note that these functions produce different outcomes.
