...
Over the next few lessons, we'll start learning how to transform _data_ into _information_. While the technical details of achieving this goal will require practice to master, the goal should always be kept in mind first and foremost.

Below is a short case study on the [1986 Space Shuttle Challenger explosion](https://en.wikipedia.org/wiki/Space_Shuttle_Challenger_disaster). In this example, we'll be examining data about the relationship between the temperature of different NASA rocket launches and damage to the rock O-rings - the root cause of the accident. Don't worry yet if you don't understand the specific code used - just try to follow along to see how we can pull out _information_ from raw data.
